...
static int io_callback(sd_event_source *s, int fd, uint32_t revents, void *userdata) {
        ...
        if ((revents & EPOLLOUT) && iov->iov_len > 0) {
                ssize_t n;
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
        ...
        /* Free these explicitly so that they are freed before the fds */
        client_event_source = sd_event_source_unref(client_event_source);
        server_event_source = sd_event_source_unref(server_event_source);
